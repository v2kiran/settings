...
if ($psveriontable.psversion.major -eq 5)
{
    if ($env:wt_session)
    {
        # theme AdventureTime
...
# user defined
# PSreadline logs everything by default including passwords and credentials
# use the option below to exclude logging sensitive information
...
Set-PSReadLineKeyHandler -Key Ctrl+Alt+c -BriefDescription 'OpenInCode' -ScriptBlock {
    [Microsoft.PowerShell.PSConsoleReadLine]::RevertLine()
